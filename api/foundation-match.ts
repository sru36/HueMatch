...
export default function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const confidence = Math.max(
      0,
      Math.min(100, Math.round((1 - bestMatch.distance / maxDistance) * 100)),
    );
    ...
    if (confidence < 70) {
      recommendations.push(
        "Consider trying the shade in natural lighting before purchasing",
      );
    }

    if (userUndertone !== bestMatch.undertone) {
      recommendations.push(
        `Your skin appears to have ${userUndertone} undertones, but we matched you with a ${bestMatch.undertone} shade. You might also want to try shades from the ${userUndertone} range.`,
      );
    }

    recommendations.push(
      "For best results, test the foundation on your jawline in natural daylight",
    );
    recommendations.push(
      "This project is based on MAC Studio Fix Fluid or Pro Longwear formulations shades. https://www.maccosmetics.in/products/face/foundations",
    );
